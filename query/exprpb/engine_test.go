...
func TestPlan(t *testing.T) {
	...
	plan, err := builder.planBuilder.Build()
	require.NoError(t, err)
	require.Equal(t, "bar", plan.SchemaScan.TableName)
	...
	plan, err = builder.planBuilder.Build()
	require.NoError(t, err)
	require.Equal(t, &logicalplan.Column{ColumnName: "foo"}, plan.Projection.Exprs[0])
	require.Equal(t, "bar", plan.Input.TableScan.TableName)
