...
func TestTable(t *testing.T) {
	...
	err = table.View(func(tx uint64) error {
		...
		return table.Iterator(context.Background(), tx, pool, as, nil, nil, nil, &tIteratorProvider{
...
func Test_Table_GranuleSplit(t *testing.T) {
	...
	err = table.View(func(tx uint64) error {
		...
		return table.Iterator(context.Background(), tx, pool, as, nil, nil, nil, &tIteratorProvider{callback: func(r arrow.Record) error {
			defer r.Release()
			t.Log(r)
			return nil
		}})
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			err := table.View(func(tx uint64) error {
				...
				err = table.Iterator(context.Background(), tx, pool, as, nil, nil, nil, &tIteratorProvider{callback: func(ar arrow.Record) error {
					mtx.Lock()
...
func benchmarkTableInserts(b *testing.B, rows, iterations, writers int) {
	...
	for i := 0; i < b.N; i++ {
		...
		err = table.View(func(tx uint64) error {
			...
			return table.Iterator(ctx, tx, pool, as, nil, nil, nil, &tIteratorProvider{callback: func(ar arrow.Record) error {
				defer ar.Release()
				totalrows += ar.NumRows()
...
func Test_Table_ReadIsolation(t *testing.T) {
	...
	err = table.View(func(tx uint64) error {
		...
		rows := int64(0)
		err = table.Iterator(context.Background(), table.db.highWatermark.Load(), pool, as, nil, nil, nil, &tIteratorProvider{callback: func(ar arrow.Record) error {
			mtx.Lock()
			rows += ar.NumRows()
			mtx.Unlock()
			defer ar.Release()
...
func Test_Table_Filter(t *testing.T) {
	...
	err = table.View(func(tx uint64) error {
		...
		err = table.Iterator(context.Background(), tx, pool, as, nil, filterExpr, nil, &tIteratorProvider{callback: func(ar arrow.Record) error {
			defer ar.Release()
...
func Test_DoubleTable(t *testing.T) {
	...
	err = table.View(func(tx uint64) error {
		...
		return table.Iterator(context.Background(), tx, pool, as, nil, nil, nil, &tIteratorProvider{callback: func(ar arrow.Record) error {
			defer ar.Release()
