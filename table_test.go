...
func Test_Table_NestedSchema(t *testing.T) {
	...
	sortCols, err := dynparquet.SortingColumnsFromDef(def)
	require.NoError(t, err)

	sc := dynparquet.ParquetSchemaFromV2Definition(def)
	buf := parquet.NewBuffer(sc, parquet.SortingColumns(sortCols...))
