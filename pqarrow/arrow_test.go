...
func TestDifferentSchemasToArrow(t *testing.T) {
	...
	require.NoError(t, c.Convert(ctx, buf0, dynSchema, nil))
	require.NoError(t, c.Convert(ctx, buf1, dynSchema, nil))
	require.NoError(t, c.Convert(ctx, buf2, dynSchema, nil))
